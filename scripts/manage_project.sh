...
if ! gh extension list | grep -q '^github/gh-project'; then
  echo "Installing gh-project extension..."
...
PROJECT_NUMBER=$(gh project list --repo "$REPO" --json number,name --jq '.[] | select(.name=="'"$PROJECT_NAME"'").number')

if [[ -z "$PROJECT_NUMBER" ]]; then
  echo "Project not found; creating project '$PROJECT_NAME'..."
  gh project create "$PROJECT_NAME" --repo "$REPO" --description "Kanban board for InkLink tasks"
