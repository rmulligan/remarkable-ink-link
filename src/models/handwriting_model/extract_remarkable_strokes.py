...
def download_notebook(rmapi_cmd: str, document_id: str, output_dir: str) -> Optional[str]:
    """
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if isinstance(rmapi_cmd, str) and "docker" in rmapi_cmd:
                ...
                # Regular rmapi command
                cmd = [rmapi_cmd, "get", document_id, f"{temp_dir}/notebook.zip", "--format", "zip"]
            ...
            if result.returncode != 0:
                logger.error(f"Failed to download notebook {document_id}: {result.stderr}")
                return None
            ...
            if result.returncode != 0:
                logger.error(f"Failed to extract notebook {document_id}: {result.stderr}")
                return None
...
def main():
    ...
    default_docker_cmd = f"docker run -v {home_dir}/.config/rmapi/:/home/app/.config/rmapi/ rmapi"
    
    parser = argparse.ArgumentParser(description="Extract strokes from reMarkable notebooks")
    parser.add_argument("--rmapi", type=str, default=default_docker_cmd,
    ...
    if not args.document_id:
        ...
        for i, notebook in enumerate(notebooks):
            print(f"[{i+1}] {notebook.get('VissibleName', 'Unnamed')} - ID: {notebook.get('ID')}")
