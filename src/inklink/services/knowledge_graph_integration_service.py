...
class KnowledgeGraphIntegrationService:
    ...
    def __init__(
        ...
        self.knowledge_graph_service = (
            knowledge_graph_service or KnowledgeGraphService()
        )
        self.remarkable_service = remarkable_service or RemarkableService()

        # Configuration
        self.min_entity_confidence = float(
            CONFIG.get("KG_MIN_ENTITY_CONFIDENCE", "0.7")
        )
        self.min_relation_confidence = float(
            CONFIG.get("KG_MIN_RELATION_CONFIDENCE", "0.7")
        )
        self.min_semantic_similarity = float(
            CONFIG.get("KG_MIN_SEMANTIC_SIMILARITY", "0.6")
        )
    ...
    def extract_knowledge_from_notebook(
        ...
        try:
            ...
            success, entities_result = (
                self.knowledge_graph_service.extract_entities_from_text(
                    text=recognized_text, entity_types=entity_types
                )
            )
            ...
            for entity in entities_result.get("entities", []):
                ...
                if entity.get("score", 0) >= self.min_entity_confidence:
                    success, entity_result = self.knowledge_graph_service.create_entity(
                        ...
                        observations=[
                            entity.get("context", "Extracted from handwritten notes")
                        ],
                    )
                    if success:
            ...
            for entity in created_entities:
                ...
                success, rel_result = (
                    self.knowledge_graph_service.extract_relationships_from_text(
            ...
            for entity in created_entities:
                entity_name = entity.get("name")
                success, links_result = (
                    self.knowledge_graph_service.create_semantic_links(
    ...
    def semantic_search_from_handwritten_query(
        ...
        try:
            logger.info(
                f"Performing semantic search from handwritten query: {rm_file_path}"
            )
            ...
            if not query_text:
                return False, {
                    "error": "Failed to recognize text from handwritten query"
                }

            # Step 2: Perform semantic search using the query text
            success, search_result = (
                self.knowledge_graph_service.find_semantically_similar_text(
                    text=query_text,
            ...
            logger.error(
                f"Error performing semantic search from handwritten query: {e}"
            )
        ...
        try:
            ...
            for entity in entities_result.get("entities", []):
                if entity.get("score", 0) >= self.min_entity_confidence:
                    ...
                    success, entity_result = self.knowledge_graph_service.get_entity(
                        entity_name
                    )
                    ...
                    if success:
                        ...
                        entity_info["observations"] = entity_result.get(
                            "properties", {}
                        ).get("observations", [])

                        # Get related entities through relationships
                        if include_related:
                            success, related_result = (
                                self.knowledge_graph_service.get_entity_relationships(
                                    entity_name=entity_name
                                )
                            )
                            if success:
                                entity_info["related_entities"] = related_result.get(
                                    "related_entities", []
                                )

                        # Get semantically similar entities
                        if include_semantic:
                            success, semantic_result = (
                                self.knowledge_graph_service.create_semantic_links(
                                    entity_name=entity_name,
                                    min_similarity=self.min_semantic_similarity,
                                    max_links=5,
                            ...
                            if success:
                                entity_info["similar_entities"] = semantic_result.get(
                                    "similar_entities", []
                                )
                    else:
