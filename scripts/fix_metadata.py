...
def update_metadata(content_file, metadata_file):
    ...
    try:
        ...
        if not metadata_file:
            metadata_file = os.path.join(os.path.dirname(content_file), f"{content_id}.metadata")
...
def create_modified_rmdoc():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Create the zip file
        modified_rmdoc = os.path.join(NOTEBOOK_DIR, f"{NOTEBOOK_NAME}_metadata_fixed.rmdoc")
...
def upload_to_remarkable(rmdoc_path):
    ...
    try:
        ...
        if NOTEBOOK_NAME not in rmdoc_path:
            ...
            id_line = next((line for line in result.stdout.splitlines() if "ID:" in line), None)
            if id_line:
