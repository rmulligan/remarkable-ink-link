...
def create_test_file():
    ...
    try:
        ...
        test_file_path = tempfile.mktemp(suffix='.pdf')
...
def upload_test_file(test_file_path):
    ...
    try:
        ...
        list_result = subprocess.run(list_cmd, shell=True, check=True, capture_output=True, text=True)
        ...
        for line in list_result.stdout.splitlines():
            if base_name in line:
                ...
                uploaded_file = line.strip().split()[-1]  # Get the last column which is the name
                break
        
        if not uploaded_file:
            # Try using the test file name as fallback
            uploaded_file = base_name
            logger.warning(f"Could not find uploaded file in listing, using {uploaded_file} as fallback")
