...
def mock_knowledge_graph_service():
    ...
    service.extract_entities_from_text.return_value = (
        True,
        {
            "count": 5,
            "entities": [
                ...
                },
                {
                    ...
                    "type": "Concept",
                    "score": 0.85,
                    "context": "ML algorithms learn from data and make predictions.",
                },
                {
                    "name": "Neural Networks",
                    "type": "Concept",
                    "score": 0.95,
                    "context": "Neural Networks are a type of ML algorithm inspired by the human brain.",
                },
                {
                    "name": "TensorFlow",
                    "type": "Product",
                    "score": 0.95,
                    "context": "TensorFlow is a framework developed by Google for implementing Neural Networks.",
                },
            ],
        },
    )
    ...
    service.extract_relationships_from_text.return_value = (
        True,
        {
            "count": 3,
            "relationships": [
    ...
    # Mock create_entity
    service.create_entity.return_value = (
        True,
        {"id": "123", "name": "Machine Learning", "type": "Concept"},
    )

    # Mock create_relationship
    service.create_relationship.return_value = (
        True,
        {
            "id": "456",
            "from": "Machine Learning",
    ...
    service.create_semantic_links.return_value = (
        True,
        {
    ...
    service.find_semantically_similar_text.return_value = (
        True,
        {
...
def test_extract_entities_from_remarkable_notebook(
    round_trip_service, mock_handwriting_service, mock_knowledge_graph_service
):
    ...
    def extract_knowledge_from_notebook(rm_file_path, knowledge_graph_service):
        ...
        iink_data = round_trip_service.handwriting_service.convert_to_iink_format(
            strokes
        )
        recognition_result = (
            round_trip_service.handwriting_service.recognize_handwriting(iink_data)
        )
        ...
        export_result = round_trip_service.handwriting_service.export_content(
            content_id, "text"
        )
        ...
        success, entities_result = knowledge_graph_service.extract_entities_from_text(
            recognized_text
        )
        if not success:
            return False, {
        ...
        for entity in created_entities:
            entity_name = entity.get("name")
            success, rel_result = (
                knowledge_graph_service.extract_relationships_from_text(
                    text=recognized_text, from_entity=entity_name
                )
            )
        ...
        for entity in created_entities:
            ...
            success, links_result = knowledge_graph_service.create_semantic_links(
                entity_name=entity_name, min_similarity=0.7, max_links=5
            )
            if success:
    ...
    # Run the test
    success, result = extract_knowledge_from_notebook(
        rm_file_path, mock_knowledge_graph_service
    )
    ...
    mock_handwriting_service.export_content.assert_called_once_with(
...
def test_semantic_search_from_handwritten_query(
    round_trip_service, mock_handwriting_service, mock_knowledge_graph_service
):
    ...
    def search_knowledge_graph_from_handwriting(rm_file_path, knowledge_graph_service):
        ...
        success, search_result = knowledge_graph_service.find_semantically_similar_text(
            text=query_text, min_similarity=0.6, max_results=10
        )
    ...
    # Run the test
    success, result = search_knowledge_graph_from_handwriting(
        rm_file_path, mock_knowledge_graph_service
    )
