...
def process_rendered_pages(rendered_pages: List[str]) -> None:
    ...
    for page in rendered_pages:
        ...
        try:
            ...
            subprocess.run(
                [script_path, page, "--content-type", "mixed", "--kg"],
                check=True
            )
        except subprocess.CalledProcessError as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Sync reMarkable notebooks to Lilly's workspace")
    ...
    parser.add_argument("--process", action="store_true", help="Process notebooks after syncing")
