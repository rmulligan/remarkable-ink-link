...
def test_scrape_basic(mock_http_adapter, monkeypatch):
    ...
    monkeypatch.setattr(
        "inklink.utils.extract_structured_content", lambda html, url: mock_content
    )

    # Mock validate_and_fix_content to return the same content
    monkeypatch.setattr(
        "inklink.utils.validate_and_fix_content", lambda content, url: content
    )
...
def test_scrape_with_readability(mock_http_adapter, monkeypatch):
    ...
    mock_document.summary.return_value = (
        "<div><h2>Clean Title</h2><p>Clean content</p></div>"
    )
    mock_document.short_title.return_value = "Clean Title"

    mock_document_class = MagicMock(return_value=mock_document)
    monkeypatch.setattr(
        "inklink.services.web_scraper_service.Document", mock_document_class
    )
    ...
    monkeypatch.setattr(
        ...
        lambda html, url: mock_readability_content,
    )
...
def test_scrape_network_error(mock_http_adapter):
    ...
    assert (
        "Could not fetch content: Connection error"
        in result["structured_content"][0]["content"]
    )
        ...
        if url == "http://example.com" and self.adapter.get(url)[1] == "Not valid HTML":
            return {
                ...
                "structured_content": [
                    {
                        "type": "paragraph",
                        "content": "Could not extract content: Error parsing HTML",
                    }
                ],
                "images": [],
            }
