...
def test_escape_hcl(document_service):
    ...
    assert document_service._escape_hcl('Test "quoted" text') == 'Test \\"quoted\\" text'
...
def test_create_hcl(document_service):
    ...
    assert f'puts "size {document_service.page_width} {document_service.page_height}"' in hcl_content
...
def test_convert_to_remarkable(mock_run, document_service):
    ...
    with NamedTemporaryFile(suffix='.hcl', dir=document_service.temp_dir, delete=False) as hcl_file:
    ...
    with open(rm_path, 'wb') as f:
        f.write(b'Mock RM file content with enough bytes to pass size check' + b'x' * 100)
