...
class CredentialManager:
    ...
    def __init__(self, 
        ...
        self.storage_dir = Path(storage_dir) if storage_dir else Path.home() / ".lilly" / "credentials"
        self.storage_dir.mkdir(parents=True, exist_ok=True)
        
        # Use either provided master password or from env var
        self._master_password = master_password or os.environ.get("LILLY_MASTER_PASSWORD", None)
        if not self._master_password:
            logger.warning("No master password provided. Falling back to default (INSECURE).")
        ...
        if salt:
            ...
            logger.warning("Using default salt. Consider setting LILLY_CREDENTIAL_SALT env var.")
