...
class GoogleDocsService:
    ...
    def _authenticate(self):
        ...
        if not all([Request, Credentials, InstalledAppFlow, build]):
            logger.warning('Google API client libraries not installed; authentication disabled.')
            return
        try:
        ...
        try:
            def export_html():
                return self.drive_service.files() \
                    ...
                    .execute()
            html_content = retry_operation(export_html, operation_name='Google Docs export')
            ...
            logger.error(format_error('googledocs', 'Failed to fetch Google Docs document', e))
            return {
                'title': url_or_id,
                'structured_content': [
                    {'type': 'paragraph',
    ...
    def _process_container(self, container: BeautifulSoup, base_url: str
        ...
        for tag in container.find_all([
            ...
            if name == 'img':
                ...
                items = [li.get_text(strip=True)
                         for li in tag.find_all('li') if li.get_text(strip=True)]
                if items:
