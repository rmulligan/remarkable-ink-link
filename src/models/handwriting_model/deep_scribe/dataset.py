...
class StrokeDataset(Dataset):
    ...
    def _extract_features(self, stroke: Dict[str, List[float]]) -> torch.Tensor:
        ...
        # Normalize
        x_norm = (x_points - np.min(x_points)) / (
            np.max(x_points) - np.min(x_points) + 1e-8
        )
        y_norm = (y_points - np.min(y_points)) / (
        ...
        if seq_length > self.max_length:
            ...
            # Pad with zeros
            padding = np.zeros(
                (self.max_length - seq_length, features.shape[1]), dtype=np.float32
            )
...
def create_synthetic_dataset(
    ...
    for i in range(num_samples):
        ...
        stroke = {"id": f"synthetic_{i}", "x": x, "y": y, "p": pressure}
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Dataset utilities for handwriting recognition"
    )
    parser.add_argument(
        "--create-synthetic", action="store_true", help="Create synthetic dataset"
    )
    parser.add_argument(
        "--output-dir",
