...
from src.inklink.services.handwriting_recognition_service import HandwritingRecognitionService
...
def create_test_strokes():
    ...
    return [
        # 'h' stroke
        {
            ...
            "t": [current_time, current_time+10, current_time+20, current_time+30, 
                  current_time+40, current_time+50, current_time+60, current_time+70]
        },
        # 'e' stroke
        {
            "id": "2",
            ...
            "t": [current_time+100, current_time+110, current_time+120, current_time+130, 
                  current_time+140, current_time+150, current_time+160, current_time+170]
        },
        # 'l' stroke
        {
            "id": "3",
            ...
            "y": [100, 120, 140, 160],
            "p": [0.5, 0.6, 0.7, 0.5],
            "t": [current_time+200, current_time+210, current_time+220, current_time+230]
        },
        # 'l' stroke
        {
            "id": "4",
            ...
            "y": [100, 120, 140, 160],
            "p": [0.5, 0.6, 0.7, 0.5],
            "t": [current_time+300, current_time+310, current_time+320, current_time+330]
        },
        # 'o' stroke
        {
            "id": "5",
            ...
            "t": [current_time+400, current_time+410, current_time+420, current_time+430, 
                  current_time+440, current_time+450, current_time+460]
        }
    ]
...
class WebAdapterTester:
    ...
    def __init__(self):
        ...
        logger.info(f"Using MyScript App Key: {self.app_key[:8]}...{self.app_key[-8:] if len(self.app_key) > 16 else ''}")
        logger.info(f"Using MyScript HMAC Key: {self.hmac_key[:8]}...{self.hmac_key[-8:] if len(self.hmac_key) > 16 else ''}")
    ...
    def test_convert_to_iink_format(self):
        ...
        success = (
            "contentType" in iink_data and
            "strokeGroups" in iink_data and
            len(iink_data["strokeGroups"]) > 0 and
            "strokes" in iink_data["strokeGroups"][0] and
            len(iink_data["strokeGroups"][0]["strokes"]) == len(test_strokes)
        )
        
        if success:
            logger.info(f"✅ Conversion successful - {len(test_strokes)} strokes converted")
        else:
    ...
    def test_file_processing(self):
        ...
        if not test_file_path:
            logger.error("❌ Cannot continue with file processing test - failed to create test file")
            return False, None
    ...
    def test_service_integration(self):
        ...
        if result.get("success", False):
            ...
            logger.error(f"❌ Service integration failed: {result.get('error', 'Unknown error')}")
            return False, result
    ...
    def run_tests(self):
        ...
        print("\n" + "=" * 80)
        if all_passed:
            print("🎉 All tests passed! MyScript Web API integration is working correctly!")
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skip-file-test", action="store_true", help="Skip .rm file test (useful if rmscene is not installed)")
    ...
    if args.skip_file_test:
        ...
        if ping_result and format_result:
            ...
            print(f"Format Conversion Test: {'✅ PASSED' if format_result else '❌ FAILED'}")
            print(f"Direct Recognition Test: {'✅ PASSED' if recognition_result else '❌ FAILED'}")
            print(f"Service Integration Test: {'✅ PASSED' if service_result else '❌ FAILED'}")
