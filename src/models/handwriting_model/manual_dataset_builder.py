...
def visualize_strokes(
    strokes: List[Dict[str, Any]], labels: Optional[List[str]] = None
) -> None:
    """
...
def create_character_dataset(
    strokes_file: str, transcription: str, output_dir: str
) -> Dict[str, str]:
    """
...
def create_character_dataset_batch(
    strokes_file: str, transcription: str, output_dir: str
) -> Dict[str, str]:
    """
...
def create_word_dataset(
    strokes_file: str, words: List[str], output_dir: str
) -> Dict[str, str]:
    """
    ...
    print(f"Total strokes: {len(strokes)}")
    print(
        "For each word, enter the stroke indices that make up the word, separated by spaces."
    )
    ...
    for word_idx, word in enumerate(words):
        ...
        try:
            ...
            labels[word_id] = word
            logger.info(
                f"Saved word '{word}' with {len(indices)} strokes to {stroke_file}"
            )
...
def create_dataset_from_file(
    rm_file: str, text_file: str, output_dir: str, dataset_type: str
) -> None:
    """
...
def process_notebook_with_transcriptions(
    notebook_path: str, transcriptions_file: str, output_dir: str, dataset_type: str
) -> None:
    """
        ...
        if i < len(transcriptions):
            ...
            if dataset_type == "characters":
                create_character_dataset_batch(
                    str(rm_file), transcriptions[i], page_output_dir
                )
            elif dataset_type == "words":
                words = transcriptions[i].split()
                create_word_dataset(str(rm_file), words, page_output_dir)
    ...
    logger.info(
        f"Saved combined labels with {len(all_labels)} entries to {combined_labels_file}"
    )
...
def main():
    ...
    parser.add_argument(
        "--text-file", type=str, help="Path to the text file with transcription"
    )
