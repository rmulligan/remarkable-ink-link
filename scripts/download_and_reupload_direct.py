...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        success, stdout, stderr = run_rmapi_command(f"get \"{notebook_name}\"", cwd=temp_dir)
        ...
        if doc_id:
            ...
            success, stdout, stderr = run_rmapi_command(f"mv \"{doc_id}\" \"{modified_name}\"")
            if success:
                logger.info(f"Successfully renamed uploaded notebook to {modified_name}")
            else:
