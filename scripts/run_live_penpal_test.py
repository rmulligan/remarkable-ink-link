...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Run a full live test of Claude Penpal Service")
    parser.add_argument("--tag", type=str, default="Lilly", help="Tag to look for in notebooks")
    parser.add_argument("--wait", type=int, default=120, help="How long to wait for processing (seconds)")
...
def setup_environment(args):
    ...
    claude_command = args.claude_command or CONFIG.get("CLAUDE_COMMAND", "/home/ryan/.claude/local/claude")
...
def run_live_test(args, rmapi_path, claude_command):
    ...
    if not verify_tagged_notebooks(rmapi_adapter, args.tag):
        logger.error(f"No notebooks found with tag '{args.tag}'. Please create a notebook with a '{args.tag}' tag.")
        return False
    ...
    try:
        ...
        while time.time() - start_time < wait_time:
            ...
            if processed_count > 0:
                logger.info(f"✅ Success! Processed {processed_count} page(s) after {elapsed} seconds")
                return True
            
            # Update status every 10 seconds
            if elapsed % 10 == 0:
                logger.info(f"Waiting... {elapsed}s elapsed, {remaining}s remaining, {processed_count} pages processed")
...
def main():
    ...
    if success:
        ...
        logger.info("The Claude Penpal Service successfully processed at least one notebook page")
