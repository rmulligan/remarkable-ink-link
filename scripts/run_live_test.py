...
"""
    ...
    def _insert_response_after_query(self, *args, **kwargs):
        ...
        try:
            ...
            notebook_path = kwargs.get("notebook_path", args[2] if len(args) > 2 else None)
            
            # Find and extract content
            if notebook_path and os.path.exists(notebook_path) and zipfile.is_zipfile(notebook_path):
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    if metadata_file_path and os.path.exists(metadata_file_path):
                        with open(metadata_file_path, 'r') as f:
                            ...
                            if self.verbose:
                                logger.info(f"Original metadata: {json.dumps(self.last_metadata, indent=2)}")
            ...
            try:
                ...
                self.method_calls.append(("_insert_response_after_query", f"ERROR: {str(e)}"))
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if not content_file:
                ...
                if metadata_file:
                    ...
                    metadata_file = os.path.join(extract_dir, f"{notebook_uuid}.metadata")
            ...
            if not pages:
                ...
                if "Lilly" not in first_page["tags"]:
                    ...
                    logger.info(f"Added Lilly tag to existing page: {first_page.get('id')}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Complete live test for Claude Penpal Service")
    parser.add_argument("--notebook", type=str, default="Test_Claude_Penpal_Notebook", help="Notebook to use for testing")
    parser.add_argument("--tag", type=str, default="Lilly", help="Tag to use for query pages")
    parser.add_argument("--rmapi-path", type=str, help="Path to rmapi executable")
    parser.add_argument("--verbose", "-v", action="store_true", help="Enable verbose logging")
    parser.add_argument("--skip-tagging", action="store_true", help="Skip notebook tagging step")
    parser.add_argument("--timeout", type=int, default=120, help="Maximum timeout in seconds")
    ...
    if not args.skip_tagging:
        ...
        logger.info(f"Successfully tagged notebook {args.notebook} with HasLilly and Lilly tags")
