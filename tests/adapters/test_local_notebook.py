...
def process_local_notebook():
    ...
    class MockRmapiAdapter:
        ...
        def list_files(self):
            return True, [{"ID": NOTEBOOK_NAME, "VissibleName": NOTEBOOK_NAME, "Type": "DocumentType"}]
            
        def find_tagged_notebooks(self, tag, pre_filter_tag=None):
            return [{"id": NOTEBOOK_NAME, "name": NOTEBOOK_NAME, "tags": ["Subject:Work", "HasLilly"]}]
        ...
        def upload_file(self, file_path, title):
            ...
            response_dir = os.path.join(NOTEBOOK_DIR, "responses", datetime.now().strftime("%Y%m%d_%H%M%S"))
...
"""
    ...
    notebook = {"ID": NOTEBOOK_NAME, "VissibleName": NOTEBOOK_NAME, "Type": "DocumentType"}
