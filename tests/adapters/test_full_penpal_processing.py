...
def create_test_rm_file_with_query():
    ...
    def create_stroke(points):
        ...
        points_data = b''
        for x, y in points:
            point = struct.pack('<fffff', x, y, 1.0, 0.0, 0.0)  # x, y, pressure, rotation, tilt
            points_data += point
    ...
    strokes_data += create_stroke([
        (250, 120), (240, 100), (260, 100), (270, 120),
        (270, 150), (270, 170)
    ])  # Curve and stem
...
def create_penpal_test_notebook():
    ...
    temp_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "test_penpal_notebook")
...
def main():
    ...
    parser.add_argument("--use-real-claude", action="store_true", help="Use real Claude instead of mock")
