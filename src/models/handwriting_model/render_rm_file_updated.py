    ...
    try:
        ...
        for block in scene_tree.blocks:
            ...
            if hasattr(block, 'points') and block.points:
                points = [(p.x, p.y, p.pressure if hasattr(p, 'pressure') else 1.0) 
                          for p in block.points]
                
                stroke = {
                    'points': points,
                    'width': block.brush_thickness if hasattr(block, 'brush_thickness') else 2.0,
                ...
                for item in block.items:
                    if hasattr(item, 'points') and item.points:
                        ...
                        stroke = {
                            'points': points,
                            'width': item.brush_thickness if hasattr(item, 'brush_thickness') else 2.0,
                            'color': (0, 0, 0)  # Default color
                        }
                        strokes.append(stroke)
    except Exception as e:
