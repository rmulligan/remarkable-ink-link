...
def parse_args():
    ...
    parser.add_argument("--notebook-name", type=str, help="Use a specific notebook name")
...
def get_test_notebook(rmapi_adapter, notebook_id=None, notebook_name=None):
    ...
    try:
        ...
        if notebook_id:
            test_notebook = next((nb for nb in notebooks if nb.get("ID") == notebook_id), None)
        ...
        # Try to find by name if provided
            ...
            test_notebook = next((nb for nb in notebooks if nb.get("VissibleName") == notebook_name), None)
            if not test_notebook:
        ...
        # Otherwise use the first notebook that's a document (not a collection)
            ...
            test_notebook = next((nb for nb in notebooks if nb.get("Type") == "DocumentType"), None)
            if not test_notebook:
...
def test_original_metadata(notebook_data):
    ...
    try:
        ...
        content = notebook_data["content"].copy()  # Create a copy to avoid modifying original
        metadata = notebook_data["metadata"].copy()  # Create a copy to avoid modifying original
        ...
        original_dir = os.path.join(os.path.dirname(notebook_data["zip_path"]), "original")
        os.makedirs(original_dir, exist_ok=True)
        
        # Save modified content and metadata to new files
        original_content_path = os.path.join(original_dir, os.path.basename(content_file_path))
        original_metadata_path = os.path.join(original_dir, os.path.basename(metadata_file_path))
        ...
        with zipfile.ZipFile(original_zip_path, 'w') as zipf:
            for file_path in [original_content_path, original_metadata_path, page_file_path]:
                zipf.write(file_path, os.path.basename(file_path))
        
        logger.info(f"Created test notebook with original metadata at {original_zip_path}")
...
def test_fixed_metadata(notebook_data):
    ...
    try:
        ...
        # Save modified content and metadata to new files
        fixed_content_path = os.path.join(fixed_dir, os.path.basename(content_file_path))
        fixed_metadata_path = os.path.join(fixed_dir, os.path.basename(metadata_file_path))
...
def upload_and_test(rmapi_adapter, zip_path, approach, notebook_name):
    ...
    try:
        # Upload the notebook
        success, message = rmapi_adapter.upload_file(zip_path, f"{notebook_name}_{approach}")
...
def verify_metadata_fix(args, rmapi_path):
    ...
    try:
        ...
        if fixed_success and not original_success:
            logger.info("✅ VERIFICATION PASSED: Fixed metadata approach works while original fails")
            logger.info("This confirms that the metadata fix resolves the HTTP 400 errors!")
            return True
        elif fixed_success and original_success:
            logger.info("⚠️ INCONCLUSIVE: Both approaches work")
            logger.info("The original approach may be working due to changes in the reMarkable API or other factors")
            return True
            ...
            logger.error("❌ VERIFICATION FAILED: Original approach works but fixed approach fails")
