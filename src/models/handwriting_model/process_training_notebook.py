...
def segment_strokes_by_line(strokes: List[Dict[str, Any]]) -> List[List[Dict[str, Any]]]:
    """
    ...
    for i, line in enumerate(lines):
        ...
        line_with_x = [(stroke, min(stroke['x']) if stroke['x'] else 0) for stroke in line]
...
def segment_strokes_by_word(line_strokes: List[Dict[str, Any]]) -> List[List[Dict[str, Any]]]:
    """
...
def process_handwriting_page(page_file: str, transcription: str, output_dir: str, 
                             dataset_type: str = "characters") -> Tuple[List[str], Dict[str, str]]:
    """
            ...
            for word_idx, word in enumerate(words):
                ...
                for stroke_idx, stroke in enumerate(word):
                    if transcription_index < len(transcription):
                        ...
                        while char.isspace() and transcription_index + 1 < len(transcription):
...
def process_notebook(notebook_path: str, output_dir: str, dataset_type: str = "characters",
                     transcriptions: Optional[Dict[int, str]] = None) -> None:
    """
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="data/remarkable",
                       help="Directory to save extracted strokes")
    parser.add_argument("--dataset-type", type=str, choices=["characters", "words", "lines"],
                       default="characters", help="Type of dataset to create")
    parser.add_argument("--transcriptions-file", type=str,
