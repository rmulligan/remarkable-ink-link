...
def mock_train_and_save(output_dir: str, vocab_size: int = 128):
    ...
    model = HandwritingRecognitionSystem(device=device, vocab_size=vocab_size)
    ...
    checkpoint = {
        ...
        "metadata": {"description": "Mock model for testing", "version": "0.1.0"},
    }
...
def main():
    ...
    print(
        f"python remarkable_integration.py test_strokes.json --model {checkpoint_path}"
    )
