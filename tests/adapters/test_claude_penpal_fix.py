...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Test Claude Penpal Service with real notebooks")
    parser.add_argument("--tag", type=str, default="Lilly", help="Tag to look for in notebooks")
    parser.add_argument("--test-tag", type=str, default="LillyTest", help="Tag to add for test purposes")
...
def insert_test_response(notebook_data):
    ...
    try:
        ...
        logger.info(f"Successfully inserted test response page (ID: {response_page_id})")
...
def test_claude_penpal_service(args, rmapi_path, claude_command):
    ...
    temp_dir, notebook_data = download_and_extract_notebook(
        rmapi_adapter, 
        test_notebook.get("id"), 
        test_notebook.get("name")
    )
...
def main():
    ...
    workflow_success = test_claude_penpal_full_workflow(args, rmapi_path, claude_command)
