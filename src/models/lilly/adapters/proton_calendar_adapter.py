...
class ProtonCalendarAdapter:
    ...
    def __init__(
        ...
        if not self.username or not self.password:
            logger.warning("Proton Calendar credentials not provided. Some functionality may be limited.")
    ...
    def get_calendars(self, refresh: bool = False) -> Tuple[bool, Union[List[Dict[str, Any]], str]]:
        """
            ...
            if self._calendars is None or refresh:
                ...
                for cal in caldav_calendars:
                    try:
                        ...
                        logger.warning(f"Failed to get properties for calendar {cal.name}: {str(e)}")
    ...
    def get_calendar_by_id(self, calendar_id: str) -> Tuple[bool, Union[caldav.Calendar, str]]:
        """
    ...
    def _parse_caldav_event(self, caldav_event: caldav.Event, calendar_id: str) -> Optional[CalendarEvent]:
        """
        ...
        try:
            ...
            description = str(getattr(vevent, 'description', '')) if hasattr(vevent, 'description') else None
            location = str(getattr(vevent, 'location', '')) if hasattr(vevent, 'location') else None
            ...
            # Get status
            status = str(getattr(vevent, 'status', '')).lower() if hasattr(vevent, 'status') else None
    ...
    def get_event_by_uid(self, calendar_id: str, event_uid: str) -> Tuple[bool, Union[CalendarEvent, str]]:
        """
        ...
        try:
            ...
            logger.error(f"Error getting event {event_uid} from calendar {calendar_id}: {str(e)}")
            return False, f"Error getting event {event_uid}: {str(e)}"
    
    def create_event(self, calendar_id: str, event: CalendarEvent) -> Tuple[bool, Union[str, CalendarEvent]]:
        """
    ...
    def update_event(self, calendar_id: str, event: CalendarEvent) -> Tuple[bool, Union[str, CalendarEvent]]:
        """
        ...
        try:
            ...
            logger.error(f"Error updating event {event.uid} in calendar {calendar_id}: {str(e)}")
    ...
    def delete_event(self, calendar_id: str, event_uid: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            logger.error(f"Error deleting event {event_uid} from calendar {calendar_id}: {str(e)}")
        ...
        try:
            ...
            for event in all_events:
                ...
                if (
                    ...
                    (event.description and query in event.description.lower()) or
                    (event.location and query in event.location.lower())
                ):
    ...
    def get_today_events(self, calendar_id: Optional[str] = None) -> Tuple[bool, Union[List[CalendarEvent], str]]:
        """
        Get today's events.
    ...
    def extract_entities_from_event(self, event: CalendarEvent) -> List[Dict[str, Any]]:
        ...
        if event.organizer:
            entities.append({
                'name': event.organizer.get('name', event.organizer.get('email', '')),
                'entityType': 'Contact',
...
if __name__ == "__main__":
    ...
    parser.add_argument("--action", choices=["list-calendars", "list-events", "get-event", "search", "today", "create-event"], required=True, help="Action to perform")
    ...
    parser.add_argument("--limit", type=int, default=10, help="Maximum number of events to retrieve")
    ...
    parser.add_argument("--start", help="Event start date and time (ISO format) for create-event action")
    parser.add_argument("--end", help="Event end date and time (ISO format) for create-event action")
    parser.add_argument("--location", help="Event location for create-event action")
    parser.add_argument("--description", help="Event description for create-event action")
    ...
    try:
        if args.action == "list-calendars":
            ...
            if not args.calendar:
                status, events = adapter.get_upcoming_events(days=args.days, limit=args.limit)
            else:
                status, events = adapter.get_upcoming_events(days=args.days, limit=args.limit, calendar_id=args.calendar)
            ...
            if not args.calendar or not args.event_uid:
                print("Error: --calendar and --event-uid are required for get-event action")
                exit(1)
            ...
            status, events = adapter.search_events(args.query, calendar_id=args.calendar)
            if status:
            ...
            if status:
                ...
                for event in events:
                    print(f"- {event.summary} (Start: {event.start.strftime('%H:%M')}, End: {event.end.strftime('%H:%M')})")
                    if event.location:
                        print(f"  Location: {event.location}")
                    print()
            else:
            ...
            if not args.calendar or not args.summary or not args.start or not args.end:
                print("Error: --calendar, --summary, --start, and --end are required for create-event action")
                exit(1)
