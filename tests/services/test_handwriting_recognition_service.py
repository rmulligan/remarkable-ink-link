...
def test_recognize_multi_page_ink(recognition_service, mock_handwriting_adapter):
    ...
    mock_handwriting_adapter.recognize_multi_page_handwriting.assert_called_once_with(ink_files)
...
def test_extract_content_from_notebook(recognition_service, mock_handwriting_adapter):
    ...
    with patch("os.listdir", return_value=["1.rm", "2.rm"]):
        with patch("os.path.isfile", return_value=True):
            ...
            args = mock_handwriting_adapter.recognize_multi_page_handwriting.call_args[0][0]
            assert set(args) == set(rm_files)


def test_extract_content_from_notebook_single_page(recognition_service, mock_handwriting_adapter):
...
def test_end_to_end_with_claude_adapter(mock_handwriting_adapter):
    ...
    mock_claude_adapter.process_multiple_images.return_value = "Claude processed multiple images"
    
    # Replace the mocks in handwriting_adapter
    mock_handwriting_adapter.process_rm_file.side_effect = lambda file_path: mock_claude_adapter.process_image(
        mock_handwriting_adapter.render_rm_file(file_path)
    )
    mock_handwriting_adapter.recognize_multi_page_handwriting.side_effect = lambda file_paths: mock_claude_adapter.process_multiple_images(
    ...
    # Assert
    assert mock_handwriting_adapter.render_rm_file.call_count >= 3  # Called for each file
