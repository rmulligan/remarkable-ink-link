...
def generate_stroke_for_char(char: str) -> Dict[str, List[float]]:
    ...
    if char in 'il1|':
        ...
        y = [center_y - base_size//2 + i * base_size//num_points + random.randint(-3, 3) 
        ...
        x = [center_x + int(radius * np.cos(angle)) + random.randint(-2, 2) for angle in t]
        y = [center_y + int(radius * np.sin(angle)) + random.randint(-2, 2) for angle in t]
        ...
        for i in range(zigs):
            ...
            x_segment = [center_x - base_size//2 + base_size*i/zigs + base_size*t[j]/zigs 
                         + random.randint(-3, 3) for j in range(len(t))]
            
            if i % 2 == 0:
                y_segment = [center_y - base_size//2 + base_size*t[j] 
                             + random.randint(-3, 3) for j in range(len(t))]
            else:
                y_segment = [center_y + base_size//2 - base_size*t[j] 
                             + random.randint(-3, 3) for j in range(len(t))]
        ...
        # S-shape
        t = np.linspace(0, 2*np.pi, num_points)
        x = [center_x + int((base_size//2) * np.sin(angle)) + random.randint(-3, 3) for angle in t]
        y = [center_y + int((base_size//3) * np.sin(2*angle)) + random.randint(-3, 3) for angle in t]
        ...
        for i in range(4):
            ...
            x_segment = [center_x - base_size//2 + base_size*i/4 + base_size*t[j]/4 
                         + random.randint(-2, 2) for j in range(len(t))]
            
            if i % 2 == 0:
                y_segment = [center_y + base_size//2 - base_size*t[j]/2 
                             + random.randint(-2, 2) for j in range(len(t))]
            else:
                y_segment = [center_y - base_size//2 + base_size*t[j]/2 
                             + random.randint(-2, 2) for j in range(len(t))]
        ...
        # 7-shape
        x1 = np.linspace(center_x - base_size//2, center_x + base_size//2, num_points // 2)
        ...
        y2 = np.linspace(center_y - base_size//2, center_y + base_size//2, num_points // 2)
    ...
    pressure = [0.3 + 0.5 * np.sin(t[i] * np.pi) + random.uniform(-0.1, 0.1) for i in range(len(t))]
    ...
    for char in CHARS:
        ...
        for i in range(num_samples_per_char):
            ...
            if i == 0:
                ...
                plt.scatter(stroke["x"], stroke["y"], c=stroke["p"], s=50, cmap="viridis", alpha=0.7)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--samples-per-char", type=int, default=5,
