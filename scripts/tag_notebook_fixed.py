...
def tag_notebook(notebook_name):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if not content_file_path:
            ...
            if os.path.exists(metadata_file_path):
                ...
                metadata = {
                    ...
                    "parent": "",
                    "lastModified": str(
                        int(os.path.getmtime(content_file_path) * 1000)
                    ),
        ...
        if not success:
            ...
            if document_id:
                logger.info(
                    f"Uploaded file with ID: {document_id}, trying to rename it"
                )
                success, _, stderr = run_rmapi_command(
                    f"mv {document_id} {notebook_name}"
                )
                if success:
                    logger.info(
                        f"Successfully renamed uploaded file to: {notebook_name}"
                    )
                    logger.info(
...
if __name__ == "__main__":
    ...
    if tag_notebook(notebook_name):
        print(
            f"Successfully tagged notebook '{notebook_name}' with HasLilly and Lilly tags."
        )
    else:
