...
class RemarkableLSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        h0 = torch.zeros(self.lstm_n_layers * self.num_dir, x.size(0), self.lstm_hidden_dim).to(x.device)
        c0 = torch.zeros(self.lstm_n_layers * self.num_dir, x.size(0), self.lstm_hidden_dim).to(x.device)
...
class CharacterPredictor:
    ...
    def preprocess_stroke(self, stroke: Dict[str, List[float]]) -> torch.Tensor:
        ...
        # Normalize
        x_points = (x_points - np.min(x_points)) / (np.max(x_points) - np.min(x_points) + 1e-8)
        y_points = (y_points - np.min(y_points)) / (np.max(y_points) - np.min(y_points) + 1e-8)
