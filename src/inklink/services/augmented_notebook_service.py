...
class AugmentedNotebookService:
    ...
    def __init__(
        ...
        self.kg_integration_service = (
            kg_integration_service
            or KnowledgeGraphIntegrationService(
            ...
            if extract_knowledge:
                ...
                if success:
                    knowledge_results = {
                        ...
                        "relationships": extract_result.get(
        ...
        try:
            ...
            for line in lines:
                if "?" in line or any(
                    ...
                    for keyword in ["find", "search", "get", "tell me", "explain"]
                ):
                    requests.append(
                        {
    ...
    def _process_with_claude_code(
        ...
        context = {
            ...
            "knowledge_graph": {
                "entities": [
                    e.get("name") for e in knowledge_results.get("entities", [])
                ],
        ...
        if requests:
            ...
            if any(
                keyword in primary_request.lower()
            ...
            try:
                success, kg_result = (
                    self.knowledge_graph_service.find_semantically_similar_text(
                        text=primary_request, min_similarity=0.6, max_results=5
                    )
                )
        ...
        response = self.ai_service.generate_text(
            prompt=user_prompt, system_prompt=system_prompt, max_tokens=4000
        )
    ...
    def _extract_sources(text: str) -> List[Dict[str, Any]]:
        ...
        for ref in refs:
            ref_num = ref.strip("[]")
            ref_content_pattern = re.compile(
                rf"\[{ref_num}\]:\s*(.*?)(?:\n\[\d+\]:|$)", re.DOTALL
            )
            match = ref_content_pattern.search(text)

            if match:
                sources.append({"type": "reference", "value": match.group(1).strip()})
