...
@pytest.mark.parametrize(
    ...
    [
        ...
        (b"https://example.com/valid<script>alert(1)</script>", "https://example.com/valid"),
        ...
        (b"https://github.com/repo/issue#123<click>here</click>", "https://github.com/repo/issue#123"),
        
        # Mixed content with multiple delimiters where only the first should be used
        (b"https://api.site.com/v1/endpoint<param>^version=2", "https://api.site.com/v1/endpoint"),
        (b"https://example.com^suffix<another>", "https://example.com"),
        
        # Edge cases with valid URLs that end with allowed special chars
        (b"https://example.com/search?q=test&limit=10<more>", "https://example.com/search?q=test&limit=10"),
        (b"https://sub.domain.org/path/123-456_789^invalid", "https://sub.domain.org/path/123-456_789"),
        
        # Valid URLs with fragments and query parameters followed by invalid content
        (b"https://site.com/path?param=value#fragment<inject>", "https://site.com/path?param=value#fragment"),
        (b"http://api.example.org/?q=search+term&page=1^sort=desc", "http://api.example.org/?q=search+term&page=1"),
    ],
)
