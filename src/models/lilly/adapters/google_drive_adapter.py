...
class DriveFile:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'DriveFile':
        ...
        if isinstance(modified_time, str):
            modified_time = datetime.datetime.fromisoformat(modified_time.replace('Z', '+00:00'))
...
class GoogleDriveAdapter:
    ...
    def __init__(
        ...
        if not GOOGLE_API_AVAILABLE:
            logger.warning("Google API client libraries not installed. Run: pip install google-api-python-client google-auth google-auth-oauthlib google-auth-httplib2")
        
        # Get file paths from environment variables if not provided
        self.credentials_file = credentials_file or os.environ.get("GOOGLE_CLIENT_SECRETS")
        self.token_file = token_file or os.environ.get("GOOGLE_TOKEN_FILE")
        self.service_account_file = service_account_file or os.environ.get("GOOGLE_SERVICE_ACCOUNT")
        ...
        elif not self.use_service_account and (not self.credentials_file or not self.token_file):
            logger.warning("Client credentials or token file not provided for Google Drive adapter")
        ...
        logger.info(f"Initialized GoogleDriveAdapter with {'service account' if use_service_account else 'OAuth'} authentication")
        ...
        try:
            ...
            for item in items:
                ...
                drive_file = DriveFile(
                    ...
                    parent_id=parent_id,
                    created_time=datetime.datetime.fromisoformat(item.get('createdTime', '').replace('Z', '+00:00')),
                    modified_time=datetime.datetime.fromisoformat(item.get('modifiedTime', '').replace('Z', '+00:00')),
    ...
    def get_file_by_id(self, file_id: str) -> Tuple[bool, Union[DriveFile, str]]:
        ...
        try:
            ...
            drive_file = DriveFile(
                ...
                parent_id=parent_id,
                created_time=datetime.datetime.fromisoformat(file.get('createdTime', '').replace('Z', '+00:00')),
                modified_time=datetime.datetime.fromisoformat(file.get('modifiedTime', '').replace('Z', '+00:00')),
    ...
    def search_files(self, query: str, page_size: int = 50) -> Tuple[bool, Union[List[DriveFile], str]]:
        """
    ...
    def update_file_metadata(
        ...
        try:
            ...
            logger.info(f"Updated metadata for {drive_file.name} with ID {drive_file.id}")
            return True, drive_file
        except Exception as e:
    ...
    def get_folder_id(self, folder_path: str, create_missing: bool = False) -> Tuple[bool, Union[str, DriveFile]]:
        """
    ...
    def get_shareable_link(self, file_id: str, role: str = 'reader') -> Tuple[bool, Union[str, str]]:
        """
    ...
    def get_recent_files(self, limit: int = 10) -> Tuple[bool, Union[List[DriveFile], str]]:
        """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--action", choices=["list", "search", "get", "download", "upload", "mkdir", "recent"], required=True, help="Action to perform")
    ...
    parser.add_argument("--service-account", action="store_true", help="Use service account authentication")
    ...
    try:
        if args.action == "list":
            ...
            if status:
                ...
                for file in files:
                    if file.is_folder:
                        ...
                        size_str = f"{int(file.size / 1024)} KB" if file.size else "Unknown size"
            ...
            if args.folder:
                # If folder is a path
                if '/' in args.folder:
                    status, folder_id = adapter.get_folder_id(args.folder, create_missing=True)
                    if not status:
                        print(f"Error: {folder_id}")
                        exit(1)
                else:
            ...
            if status:
                ...
                for file in files:
                    if file.is_folder:
                        print(f"📁 {file.name} (Modified: {file.modified_time.strftime('%Y-%m-%d %H:%M')})")
                    else:
                        size_str = f"{int(file.size / 1024)} KB" if file.size else "Unknown size"
                        print(f"📄 {file.name} ({size_str}) (Modified: {file.modified_time.strftime('%Y-%m-%d %H:%M')})")
                    print(f"  Link: {file.web_view_link}")
            else:
                print(f"Error: {files}")
