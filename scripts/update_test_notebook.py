    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if os.path.exists(pages_dir):
            ...
            for page_file in page_files:
                shutil.copy2(page_file, os.path.join(temp_pages_dir, os.path.basename(page_file)))
        
        # Copy other files that may have page content
        for rm_file in glob.glob(os.path.join(EXTRACTED_DIR, "*.rm")):
            shutil.copy2(rm_file, os.path.join(temp_pages_dir, os.path.basename(rm_file)))
...
def upload_to_remarkable(rmdoc_path):
    ...
    try:
        process = subprocess.run(upload_cmd, shell=True, check=True, capture_output=True, text=True)
        ...
        if process.returncode == 0:
            ...
            id_line = next((line for line in process.stdout.splitlines() if "ID:" in line), None)
            if id_line:
...
def main():
    ...
    if success:
        logger.info(f"Successfully updated notebook '{NOTEBOOK_NAME}' on reMarkable cloud")
