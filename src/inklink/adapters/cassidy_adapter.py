...
class CassidyAdapter(RmapiAdapter):
    ...
    def find_tagged_notebooks(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Parsed {len(documents)} valid document names from rmapi ls output")
            ...
            for doc_name in documents:
                ...
                try:
                    ...
                    has_tag, content_data = self._check_document_for_tag(doc_name, self.tag)
    ...
    def _check_document_for_tag(self, doc_id: str, tag: str) -> Tuple[bool, Dict[str, Any]]:
        """
    ...
    def download_notebook(self, doc_id_or_name: str, output_dir: str) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            if re.match(r'^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$', doc_id_or_name):
                extract_dir = os.path.join(output_dir, doc_id_or_name)
            else:
                # Create a unique folder for this extraction
                extract_dir = os.path.join(output_dir, f"{doc_id_or_name}_{int(time.time())}")
        ...
        try:
            ...
            if self.tag not in tags:
                ...
                with zipfile.ZipFile(zip_path, 'w') as zip_ref:
                    for root, _, files in os.walk(extract_dir):
                        for file in files:
                            if file != f"{doc_id}_updated.zip":  # Avoid adding the zip to itself
                                file_path = os.path.join(root, file)
                                arcname = os.path.relpath(file_path, extract_dir)
                                zip_ref.write(file_path, arcname)
                
                # Upload the updated notebook
                success, message = self.upload_file(zip_path, content_data.get('visibleName', doc_id))
                if not success:
    ...
    def remove_tag_from_page(self, doc_id: str, page_id: str, tag: str = None) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            for page_tag in page_tags:
                ...
                if (page_tag.get('pageId') == page_id and 
                   (page_tag.get('tag') == tag or page_tag.get('tag', '').lower() == tag.lower())):
                    tag_removed = True
                    logger.info(f"Removing tag '{page_tag.get('tag')}' from page {page_id}")
                else:
    ...
    def convert_rm_to_png(self, rm_file_path: str, output_dir: str = None) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            svg_path = os.path.join(output_dir, f"{os.path.splitext(rm_basename)[0]}.svg")
            ...
            if inkscape_path:
                ...
                try:
                    ...
                    try:
                        ...
                        return False, "Direct SVG to PNG conversion not supported. Please install Inkscape or cairosvg."
                    except Exception as e:
    ...
    def convert_notebook_pages_to_png(self, doc_id: str, output_dir: str) -> Tuple[bool, List[str]]:
        """
    ...
    def append_text_to_notebook(self, doc_id: str, page_id: str, text: str,
                             remove_tag: bool = True, append_mode: str = "new_page") -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            if append_mode == "new_page":
                ...
                logger.info(f"Added annotation to page {page_id} (simplified implementation)")
            ...
            if remove_tag:
                ...
                if page_tags is not None:
                    ...
                    for page_tag in list(page_tags):
                        if (page_tag.get('pageId') == page_id and
                           (page_tag.get('tag') == self.tag or
                            page_tag.get('tag', '').lower() == self.tag.lower())):
                            # Remove this tag
            ...
            return True, f"Successfully appended response text as {append_type} to notebook"
    ...
    def _create_text_rm_file(self, output_path: str, text: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if drawj2d_path:
                ...
                hcl_path = os.path.join(temp_dir, f"{os.path.basename(output_path)}.hcl")

                # Format the text into paragraphs
                paragraphs = text.split('\n\n')
                if len(paragraphs) == 1:  # If no double newlines, split on single newlines
                    paragraphs = text.split('\n')
                ...
                for paragraph in paragraphs:
                    if paragraph.strip():
                        ...
                        for word in words:
                            if len(current_line) + len(word) + 1 <= 60:  # 60 chars per line approx
                                if current_line:
                                    current_line += " " + word
                                else:
                                    current_line = word
                        ...
                        for line in wrapped_lines:
                            hcl_content += f'    text{{ pos={{150 {y_position}}} text="{line}"}}\n'
    ...
    def check_for_updates(self, last_check_time: Optional[float] = None) -> List[Dict[str, Any]]:
        """
        ...
        check_time = last_check_time or self.last_check_time
