...
class CassidyMonitor:
    ...
    def __init__(
        ...
        """
        self.rmapi_path = rmapi_path or os.environ.get("RMAPI_PATH") or CONFIG.get("RMAPI_PATH")
        self.adapter = adapter or CassidyAdapter(self.rmapi_path, tag)
        self.polling_interval = polling_interval
        self.output_dir = output_dir or os.path.join(CONFIG.get("TEMP_DIR", "/tmp"), "cassidy_monitor")
        self.tag = tag
    ...
    def _load_timestamps(self):
        ...
        if os.path.exists(self.timestamps_file):
            try:
                ...
                logger.info(f"Loaded {len(self.known_notebooks)} notebook timestamps from file")
    ...
    def _check_for_tagged_notebooks(self):
        ...
        try:
            ...
            if tagged_notebooks:
                logger.info(f"Found {len(tagged_notebooks)} documents with tag '{self.tag}'")
        ...
        try:
            ...
            for doc_name in documents:
                try:
                    ...
                    has_notebook_tag, notebook_content = self._check_document_for_tag(doc_name, tag)
                    
                    # Check for the tag at page level
                    tagged_pages = self._check_document_for_page_tags(doc_name, tag, notebook_content)
    ...
    def _check_document_for_tag(self, doc_name: str, tag: str) -> Tuple[bool, Dict[str, Any]]:
        """
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                logger.info(f"Downloading {doc_name} to {os.path.join(temp_dir, expected_file)}")
                ...
                if not os.path.exists(expected_file):
                    logger.error(f"Download reported success but file not found at {expected_file}")
                    
                    # Try to find any .rmdoc files in the temp directory
                    rmdoc_files = [f for f in os.listdir('.') if f.endswith('.rmdoc')]
                    if rmdoc_files:
                        logger.info(f"Found alternative downloaded file: {rmdoc_files[0]}")
                ...
                try:
                    ...
                    with zipfile.ZipFile(zip_path, 'r') as zipf:
                        content_files = [f for f in zipf.namelist() if f.endswith('.content')]
                        ...
                        try:
                            with open('/home/ryan/Cassidy/all_tags.txt', 'a') as log_file:
                                log_file.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} - Document '{doc_name}' has notebook tags: {tags}\n")
                        except Exception as e:
                            logger.error(f"Error writing to log file: {e}")
                        
                        if tag in tags:
                            logger.info(f"Document '{doc_name}' has notebook tag '{tag}'")
                            # Write to special log file
                            try:
                                with open('/home/ryan/Cassidy/tagged_docs.txt', 'a') as log_file:
                                    log_file.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} - Found document '{doc_name}' with notebook tag '{tag}'\n")
                            except Exception:
                                pass
                        ...
                        if tag.lower() == 'cass':
                            for t in tags:
                                if t.lower() == 'cass':
                                    logger.info(f"Document '{doc_name}' has tag with case variation: {t}")
                                    try:
                                        with open('/home/ryan/Cassidy/case_issues.txt', 'a') as log_file:
                                            log_file.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} - Document '{doc_name}' has tag '{t}' instead of '{tag}'\n")
                                    except Exception:
                                        pass
                                    return True, content_data  # Return true for case-insensitive matches too
    ...
    def _check_document_for_page_tags(self, doc_name: str, tag: str, content_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        ...
        if 'pages' in content_data.get('cPages', {}):
            for i, page in enumerate(content_data['cPages']['pages']):
                ...
                if page_id in tagged_page_ids:
                    ...
                    logger.info(f"Document '{doc_name}' has page {i} (ID: {page_id}) with tag '{tag}'")
                    
                    # Write to special log file
                    try:
                        with open('/home/ryan/Cassidy/tagged_pages.txt', 'a') as log_file:
                            log_file.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} - Found page {i} (ID: {page_id}) in document '{doc_name}' with tag '{tag}'\n")
                    except Exception:
                        pass
    ...
    def _process_tagged_document(self, doc: Dict[str, Any]):
        ...
        try:
            with open('/home/ryan/Cassidy/processing_documents.txt', 'a') as log_file:
                log_file.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} - Processing tagged document: {doc_name} ({doc_id})\n")
        except Exception:
            pass
    ...
    def _process_notebook(self, doc: Dict[str, Any]):
        ...
        if not success:
            logger.error(f"Failed to download and extract notebook {doc_name}: {extract_dir}")
            return
        
        # Convert pages to PNG
        success, png_paths = self.adapter.convert_notebook_pages_to_png(doc_id, notebook_dir)
        if not success:
    ...
    def _process_page(self, doc: Dict[str, Any], page: Dict[str, Any]):
        ...
        logger.info(f"Processing page {page_index} (ID: {page_id}) in document {doc_name} ({doc_id})")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                with zipfile.ZipFile(zip_path, 'r') as zipf:
                    ...
                    rm_paths = [f for f in zipf.namelist() if f.endswith('.rm') and page_id in f]
                    ...
                    output_path = os.path.join(page_dir, f"{os.path.splitext(rm_file_name)[0]}.png")
                    success, result = self.adapter.convert_rm_to_png(extracted_rm_path, page_dir)
