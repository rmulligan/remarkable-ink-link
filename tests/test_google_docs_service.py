...
def test_fetch_success(google_docs_service, mock_adapter):
    ...
    result = google_docs_service.fetch(
        "https://docs.google.com/document/d/test_doc_123/edit"
    )

    # Verify adapter methods were called
    assert (
        mock_adapter.extract_doc_id_calls[-1]
        == "https://docs.google.com/document/d/test_doc_123/edit"
    )
...
def test_fetch_metadata_failure(google_docs_service, mock_adapter):
    ...
    assert (
        "Could not fetch Google Docs document"
        in result["structured_content"][0]["content"]
    )
    assert (
        "Failed to get document metadata" in result["structured_content"][0]["content"]
    )
