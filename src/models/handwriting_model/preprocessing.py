...
def normalize_strokes(strokes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for stroke in strokes:
        ...
        for key in stroke:
            norm_stroke[key] = (
                stroke[key].copy() if isinstance(stroke[key], list) else stroke[key]
            )
...
def compute_stroke_features(strokes: List[Dict[str, Any]]) -> np.ndarray:
    ...
    for i, stroke in enumerate(strokes):
        ...
        assert (
            len(x_points) == len(y_points) == len(pressures)
        ), "Stroke points must have same length"
...
def strokes_to_tensor(
    strokes: List[Dict[str, Any]], max_length: int = 2000
) -> torch.Tensor:
    """
    ...
    if features_tensor.size(1) > max_length:
        ...
        padding = torch.zeros(
            (1, max_length - features_tensor.size(1), 5), dtype=torch.float32
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        # Print statistics
        print(
            f"Extracted {len(strokes)} strokes with {sum(len(s['x']) for s in strokes)} points"
        )
    else:
