...
def main():
    ...
    parser.add_argument(
        "--query-tag", default="Lilly", help="Tag to identify query pages"
    )
    parser.add_argument(
        "--context-tag", default="Context", help="Tag to identify context pages"
    )
    parser.add_argument(
        "--subject-tag", default="Subject", help="Tag prefix for subject classification"
    )
    parser.add_argument(
        "--default-subject", default="General", help="Default subject if none specified"
    )
    parser.add_argument(
        "--use-subject-dirs",
        action="store_true",
        default=True,
        help="Organize notebooks by subject",
    )
    parser.add_argument(
        "--pre-filter-tag",
        default="HasLilly",
        help="Document-level tag for pre-filtering",
    )
    parser.add_argument(
        "--no-pre-filter", action="store_true", help="Disable pre-filtering"
    )
    parser.add_argument(
    ...
    logger.info(
        f"  Pre-filter tag: {None if args.no_pre_filter else args.pre_filter_tag}"
    )
