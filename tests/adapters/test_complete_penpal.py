...
def create_test_rm_file():
    ...
    # Combine all data
    rm_data = (
        header + layers_data + strokes_data + stroke_header + points_data + all_points
    )
...
def create_test_notebook_with_query():
    ...
    temp_dir = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "test_notebook_complete"
    )
    ...
    try:
        ...
        content = {
            ...
            "originalPageCount": -1,
            "pageCount": 1,
            "pageTags": {page_id: ["Lilly"]},  # Tag the page with Lilly
            "pages": [page_id],
            "sizeInBytes": 1000,
            "tags": [],
            "textAlignment": "left",
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Complete test for Claude Penpal Service"
    )
    ...
    success, message = rmapi_adapter.upload_file(
        notebook_path, "Test_Penpal_Query_Notebook"
    )
