...
def mock_http_adapter():
    ...
    mock.get.side_effect = lambda endpoint, **kwargs: (
        ...
        if endpoint == "/v1/lifelogs" and not kwargs.get("params", {}).get("cursor")
            ...
            (True, {"data": SAMPLE_LIFE_LOGS[1:], "pagination": {}})
            if endpoint == "/v1/lifelogs"
            and kwargs.get("params", {}).get("cursor") == "cursor1"
            else (
                (True, {"data": SAMPLE_LIFE_LOGS[0]})
                if endpoint.startswith("/v1/lifelogs/")
                and SAMPLE_LIFE_LOGS[0]["id"] in endpoint
...
@pytest.fixture
def mock_limitless_controller(
    mock_limitless_service, mock_limitless_scheduler, mock_controller
):
...
class TestLimitlessMockIntegration:
    ...
    def test_adapter_pagination(self, mock_limitless_adapter, mock_http_adapter):
        ...
        response1 = {
            "data": SAMPLE_LIFE_LOGS[:1],
            "pagination": {"next_cursor": "next_page"},
        }
    ...
    def test_service_sync_logs(
        self, mock_limitless_service, mock_limitless_adapter, mock_http_adapter
    ):
    ...
    def test_scheduler_trigger_sync(
        self, mock_limitless_scheduler, mock_limitless_service
    ):
        """Test manually triggering a sync."""
        # Replace sync_life_logs with mock to avoid actual processing
        mock_limitless_service.sync_life_logs = Mock(
            return_value=(True, "Mock sync success")
        )
        ...
        mock_limitless_service.sync_life_logs.assert_called_once_with(
    ...
    def test_controller_handle_status(
        self,
        mock_limitless_controller,
        mock_limitless_service,
        mock_limitless_scheduler,
    ):
        ...
        mock_limitless_service.get_sync_status = Mock(
            return_value={"test": "service_status"}
        )
        mock_limitless_scheduler.get_status = Mock(
            return_value={"test": "scheduler_status"}
        )
    ...
    def test_controller_handle_scheduler(
        ...
        # Test start action
        result = mock_limitless_controller.handle_scheduler(
            "POST", {"action": "start"}, {}
        )
...
class TestLimitlessLiveIntegration:
    ...
    def test_full_sync_flow(
        self,
        live_limitless_adapter,
        live_limitless_service,
        live_knowledge_graph_service,
    ):
...
class TestLimitlessHTTPAPI:
    ...
    def test_sync_endpoint(self):
        ...
        response = requests.post(
            f"{self.BASE_URL}/limitless/sync", json={"force_full_sync": True}
        )
    ...
    def test_get_life_log_endpoint(self):
        ...
        response = requests.get(
            "https://api.limitless.ai/v1/lifelogs?limit=1", headers=headers
        )
