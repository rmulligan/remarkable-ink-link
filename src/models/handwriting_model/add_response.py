...
def add_response_to_svg(svg_path, response_text, output_svg_path=None, output_png_path=None, start_y=300):
    ...
    if not main_group:
        ...
        main_group = ET.SubElement(root, '{http://www.w3.org/2000/svg}g', {'id': 'claude_response'})
    ...
    response_group = ET.SubElement(main_group, '{http://www.w3.org/2000/svg}g', {'id': 'claude_response'})
    ...
    for i, line in enumerate(lines):
        ...
        line_y = y_position + i * line_height + random.uniform(-line_variation, line_variation)
        ...
        if points:
            polyline = ET.SubElement(response_group, '{http://www.w3.org/2000/svg}polyline', {
...
def main():
    parser = argparse.ArgumentParser(description='Add response text to a reMarkable notebook SVG file')
    ...
    parser.add_argument('-y', '--y-position', type=int, default=300, help='Y position to start response text')
