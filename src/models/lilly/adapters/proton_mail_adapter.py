...
class ProtonMailAdapter:
    ...
    def __init__(
        ...
        if not self.username or not self.password:
            logger.warning("Proton Mail credentials not provided. Some functionality may be limited.")
        ...
        logger.info(f"Initialized ProtonMailAdapter for {self.username} using Proton Bridge")
    ...
    def fetch_email(self, uid: int, folder: str = "INBOX") -> Tuple[bool, Union[EmailMessage, str]]:
        """
    ...
    def _parse_email(self, email_message: email.message.Message, uid: int, folder: str) -> EmailMessage:
        """
    ...
    def get_recent_emails(self, folder: str = "INBOX", limit: int = 10) -> Tuple[bool, Union[List[EmailMessage], str]]:
        """
    ...
    def search_by_keyword(self, keyword: str, folder: str = "INBOX", limit: int = 10) -> Tuple[bool, Union[List[EmailMessage], str]]:
        """
    ...
    def get_unread_emails(self, folder: str = "INBOX", limit: int = 10) -> Tuple[bool, Union[List[EmailMessage], str]]:
        """
    ...
    def move_email(self, uid: int, source_folder: str, destination_folder: str) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            if status != 'OK':
                return False, f"Failed to copy email {uid} to {destination_folder}: {data}"
            ...
            return True, f"Email {uid} moved from {source_folder} to {destination_folder}"
        except Exception as e:
    ...
    def extract_entities_from_email(self, email_message: EmailMessage) -> List[Dict[str, Any]]:
        """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--action", choices=["list-folders", "list-emails", "search", "read", "send"], required=True, help="Action to perform")
    parser.add_argument("--folder", default="INBOX", help="Mail folder to use")
    parser.add_argument("--limit", type=int, default=10, help="Maximum number of emails to retrieve")
    ...
    try:
        if args.action == "list-folders":
            ...
            if status:
                ...
                for email in emails:
                    print(f"- {email.uid}: {email.subject} (from: {email.sender}, date: {email.date})")
            else:
            ...
            status, emails = adapter.search_by_keyword(args.query, args.folder, args.limit)
            if status:
                print(f"Found {len(emails)} emails matching '{args.query}' in {args.folder}:")
                for email in emails:
                    print(f"- {email.uid}: {email.subject} (from: {email.sender}, date: {email.date})")
            else:
                print(f"Error: {emails}")
