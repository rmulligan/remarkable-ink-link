...
class ServiceProvider:
    ...
    def register(self, interface_class: Type[T], implementation_class: Type[T]) -> None:
        ...
        if interface_class in self._factories:
            logger.warning(f"Overriding existing registration for {interface_class.__name__}")
            
        # Store as a factory function
        self._factories[interface_class] = lambda: self._create_instance(implementation_class)
        ...
        for param_name, param in list(signature.parameters.items())[1:]:
            ...
            if param.annotation != inspect.Parameter.empty:
                try:
                    ...
                    logger.error(f"Cannot resolve dependency {param_name} of type {param.annotation}")
                    raise Exception(f"Cannot resolve dependency {param_name} of type {param.annotation}")
            ...
            logger.error(f"Cannot resolve parameter {param_name} for {class_type.__name__}")
            raise Exception(f"Cannot resolve parameter {param_name} for {class_type.__name__}")
    ...
    def create_from_config(config: Dict[str, Any]) -> 'ServiceProvider':
        ...
        for interface_name, implementation_name in services_config.items():
            try:
                ...
                implementation_module = importlib.import_module(".".join(implementation_parts[:-1]))
                implementation_class = getattr(implementation_module, implementation_parts[-1])
                
                # Register implementation
                provider.register(interface_class, implementation_class)
                
            except Exception as e:
                logger.error(f"Failed to register {interface_name} -> {implementation_name}: {str(e)}")
