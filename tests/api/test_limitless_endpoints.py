...
class LimitlessLiveTest:
    ...
    def test_get_life_logs(self, limit=3):
        ...
        # Print structure
        logger.info(f"API response structure: {json.dumps({k: type(v).__name__ for k, v in result.items()}, indent=2)}")
        ...
        if isinstance(result, dict):
            if "data" in result and isinstance(result["data"], list):
                logs = result["data"]
            elif "data" in result and isinstance(result["data"], dict) and "lifelogs" in result["data"]:
