...
def test_extract_strokes_manually():
    ...
    try:
        ...
        if strokes and len(strokes) > 0:
            print(f"✅ Successfully extracted {len(strokes)} strokes directly using rmscene")
            print("\nFirst stroke data:")
...
def test_adapter_extraction():
    ...
    try:
        ...
        if strokes and len(strokes) > 0:
            print(f"✅ Successfully extracted {len(strokes)} strokes using HandwritingWebAdapter")
            print("\nFirst stroke data:")
            print(json.dumps(strokes[0], indent=2))
            ...
            if all_props_present:
                ...
                missing = [prop for prop in expected_properties if prop not in strokes[0]]
