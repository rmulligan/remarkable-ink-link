...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("claude_vision_test.log")
    ]
)
...
class ClaudeVisionTester:
    ...
    def __init__(
        ...
        if not self.adapter.is_available():
            raise RuntimeError("ClaudeVisionAdapter is not available. Check Claude CLI installation.")
    ...
    def find_test_images(self, content_type: Optional[str] = None) -> List[Path]:
        ...
        logger.info(f"Found {len(images)} test images{' for ' + content_type if content_type else ''}")
        return images
    ...
    def test_single_image(self, image_path: Path, expected_content_type: Optional[str] = None) -> Tuple[bool, Dict]:
        """
        ...
        content_type = expected_content_type or self.detect_content_type_from_path(image_path)
        ...
        logger.info(f"Processed {image_path.name} in {processing_time:.2f}s: {'SUCCESS' if success else 'FAILED'}")
    ...
    def test_multi_page(
        ...
        if not content_types:
            content_types = [self.detect_content_type_from_path(path) for path in image_paths]
        ...
        for content_type in set(content_types):
            ...
            self.metrics["content_types"][content_type]["time"] += processing_time * type_fraction
        ...
        logger.info(f"Processed {len(image_paths)} pages in {processing_time:.2f}s: {'SUCCESS' if success else 'FAILED'}")
        return success, results
    ...
    def calculate_metrics(self) -> Dict:
        ...
        if metrics["page_count"] > 0:
            metrics["success_rate"] = metrics["success_count"] / metrics["page_count"] * 100
    ...
    def print_metrics(self):
        ...
        if metrics["page_count"] > 0:
            ...
            logger.info(f"Average processing time: {metrics['average_time']:.2f}s per page")
        
        logger.info("-" * 50)
        logger.info("RESULTS BY CONTENT TYPE")
        for content_type, data in metrics["content_types"].items():
            if data["count"] > 0:
                logger.info(f"{content_type.upper()}: {data['count']} pages, {data['success_rate']:.1f}% success, {data['average_time']:.2f}s avg")
        logger.info("=" * 50)
