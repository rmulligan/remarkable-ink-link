...
def convert_pdf_to_rm(pdf_path, output_rm_path=None):
    ...
    try:
        # Convert PDF to SVG
        cairosvg.svg2pdf(url=str(pdf_path), write_to=str(svg_path))
...
def main():
    parser = argparse.ArgumentParser(
        description="Create a PDF with handwritten-style text and convert to reMarkable format"
    )
    ...
    parser.add_argument(
        "-r", "--original-rm", help="Original RM file to overlay response onto"
    )
