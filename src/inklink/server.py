...
def run_server(host: str = None, port: int = None):
    ...
    try:
        test_service = RemarkableService(
            CONFIG["RMAPI_PATH"], CONFIG.get("RM_FOLDER", "/")
        )
        ...
        if not ok:
            ...
            if "Exec format error" in msg:
                ...
                if alt:
                    logger.warning(
                        "Exec format error with %s, falling back to PATH rmapi: %s",
                        CONFIG["RMAPI_PATH"],
                        alt,
                    )
                    ...
                    if ok2:
                        ...
                        logger.error(
                            f"Authentication check failed with PATH rmapi: {msg2}"
                        )
                        logger.error(
                            "Please run `inklink auth` to authenticate before sharing content."
                        )
                        import sys
                    ...
                    logger.error(
                        "No fallback rmapi found in PATH. Please install rmapi or run `inklink auth`."
                    )
                    import sys
