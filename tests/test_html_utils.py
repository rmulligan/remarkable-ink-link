...
def test_generate_title_from_url():
    ...
    assert generate_title_from_url("https://example.com/blog/my-first-post") == "My First Post - example.com"
    assert generate_title_from_url("https://test.org/") == "Page from test.org"
    assert generate_title_from_url("https://subdomain.site.net/path/with/multiple/segments") == "Segments - subdomain.site.net"
...
def test_validate_and_fix_content():
    ...
    # Count bullet items
    bullet_items = [item for item in fixed["structured_content"] if item.get("type") == "bullet"]
