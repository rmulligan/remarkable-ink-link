...
class Stroke:
    ...
    def to_dict(self):
        return {"points": self.points, "width": self.width, "color": self.color}
...
def direct_extract_strokes(file_path):
    ...
    while pos < len(data) - 24:  # Need at least 3 floats (12 bytes) + some header bytes
        ...
        if (
            current_stroke
            and len(current_stroke.points) > 0
            and (
                pos >= len(data) - 24  # End of file
                or (pos + 3 < len(data) and data[pos + 2] != 0x18)  # Pattern break
            )
        ):
            strokes.append(current_stroke.to_dict())
            current_stroke = None
...
def main():
    ...
    print(
        f"Extracted {len(strokes)} strokes with {sum(len(s['points']) for s in strokes)} points from {rm_file}"
    )
