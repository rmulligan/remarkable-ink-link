...
class LillyMonitor(CassidyMonitor):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            output_dir=output_dir or os.path.join(CONFIG.get("TEMP_DIR", "/tmp"), "lilly_monitor"),
            tag=tag,
            callback=callback
        )
        ...
        self.lilly_workspace = lilly_workspace or os.path.join(os.path.expanduser("~"), "dev", "remarkable-ink-link", "lilly")
        
        # Create Lilly-specific directories
        os.makedirs(os.path.join(self.lilly_workspace, "workspace", "remarkable_sync"), exist_ok=True)
    ...
    def process_with_claude_vision(self, image_path: str, prompt: Optional[str] = None) -> Tuple[bool, str]:
        """
    ...
    def update_knowledge_graph(self, image_path: str, claude_response: str, notebook_info: Dict[str, Any]) -> bool:
        """
        ...
        try:
            ...
            kg_script = os.path.join(self.lilly_workspace, "tools", "process_handwriting.py")
    ...
    def _process_notebook(self, doc: Dict[str, Any]):
        ...
        for png_path in png_paths:
            ...
            if not success:
                logger.error(f"Failed to process page {png_path} with Claude: {claude_response}")
            ...
            try:
                ...
                if success:
                    logger.info(f"Successfully appended response to notebook: {message}")
                else:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                with tempfile.TemporaryDirectory() as extract_dir:
                    ...
                    success, rm_path = self._extract_page_file(os.path.join(temp_dir, expected_file), page_id, extract_dir)
                    if not success:
                        logger.error(f"Failed to extract page file: {rm_path}")
                        return
                    
                    # Copy the file to the page directory
                    rm_file_in_page_dir = os.path.join(page_dir, os.path.basename(rm_path))
                    shutil.copy(rm_path, rm_file_in_page_dir)
                    
                    # Convert the page to PNG
                    png_path = os.path.join(page_dir, f"{os.path.splitext(os.path.basename(rm_path))[0]}.png")
                    ...
                    if not success:
                        logger.error(f"Failed to process page with Claude: {claude_response}")
                        return
                    ...
                    # Update the knowledge graph
                    kg_success = self.update_knowledge_graph(result, claude_response, doc)
                    if not kg_success:
                    ...
                    try:
                        ...
                        if success:
                            logger.info(f"Successfully appended response to notebook and removed tag: {message}")
                        else:
                            logger.error(f"Failed to append response to notebook: {message}")
                    except Exception as e:
                        logger.error(f"Error appending response to notebook: {str(e)}")
    ...
    def _extract_page_file(self, zip_path: str, page_id: str, extract_dir: str) -> Tuple[bool, str]:
        """
