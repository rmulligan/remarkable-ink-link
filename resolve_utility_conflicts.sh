...
# Script to resolve conflicts in utility files
# This script resolves conflicts in the utility files by preferring the origin/main version
# but keeping the necessary changes for the refactoring

# Utility files with conflicts
utility_files=(
  "src/inklink/utils/__init__.py"
  "src/inklink/utils/common.py"
  "src/inklink/utils/url_utils.py"
)
...
# For hcl_render.py, we need to manually fix it
echo "Manually fixing hcl_render.py"

cat > src/inklink/utils/hcl_render.py << 'EOF'
...
def create_hcl_from_content(
    ...
    try:
        ...
        with open(hcl_path, "w", encoding="utf-8") as f:
            ...
            f.write(f'puts "line {margin} {y_pos} {page_width - margin} {y_pos} width=1.0"\n')
            y_pos += line_height * 1.5
            ...
            if structured_content:
                ...
                for item in structured_content:
                    ...
                    if content_type == "heading":
                        ...
                        size = 36 if level == 1 else (30 if level == 2 else (24 if level == 3 else 20))
                        f.write(f'puts "set_font {heading_font} {size}"\n')
                        f.write(f'puts "text {margin} {y_pos} \\"{escape_hcl(content_value)}\\""\n')
                        y_pos += line_height * 1.5
                        ...
                        f.write(f'puts "text {margin + 20} {y_pos} \\"{escape_hcl(content_value)}\\""\n')
                        y_pos += line_height
                        ...
                        f.write(f'puts "set_font {body_font} 20"\n')
                        f.write(f'puts "text {indent} {y_pos} \\"• {escape_hcl(content_value)}\\""\n')
                        y_pos += line_height
                        ...
                        if img_path and os.path.exists(img_path):
                            ...
                            f.write(f'puts "image {margin} {y_pos} {img_width} {img_height} \\"{img_path}\\""\n')
                            y_pos += img_height + line_height
                    
                    elif content_type == "table":
                        # Basic table rendering (simplified)
                        rows = item.get("rows", [])
                        col_width = (page_width - 2 * margin) / max(len(row) for row in rows) if rows else 100
                        for row_idx, row in enumerate(rows):
                            for col_idx, cell in enumerate(row):
                                x_pos = margin + (col_idx * col_width)
                                f.write(f'puts "text {x_pos} {y_pos} \\"{escape_hcl(str(cell))}\\""\n')
                            y_pos += line_height
                ...
                f.write(f'puts "text {margin} {y_pos} \\"No content available for this page.\\""\n')

            # Add timestamp at the bottom of the page
            timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
            f.write(f'puts "set_font {body_font} 16"\n')
            f.write(f'puts "text {margin} {page_height - margin} \\"Generated: {timestamp}\\""\n')
...
git add src/inklink/utils/hcl_render.py

echo "Utility file conflicts resolved."