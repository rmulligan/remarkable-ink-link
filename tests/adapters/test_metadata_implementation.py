...
def insert_response_after_query(temp_dir, content_file_path, metadata_file_path, query_page_id):
    ...
    try:
        ...
        query_idx = next((i for i, p in enumerate(pages) if p.get("id") == query_page_id), -1)
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Test metadata handling implementation")
    parser.add_argument("--notebook-name", type=str, default="Metadata Test", help="Name for the test notebook")
    ...
    temp_dir, content_file_path, metadata_file_path, notebook_id, page_id = create_test_notebook()
    ...
    try:
        ...
        logger.info("✅ TEST PASSED: Successfully implemented and tested metadata handling fix")
        return True
