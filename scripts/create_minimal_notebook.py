...
def create_and_upload_minimal_notebook():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        debug_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "debug_upload")
        os.makedirs(debug_dir, exist_ok=True)
        debug_copy = os.path.join(debug_dir, f"{NOTEBOOK_NAME}_{int(time.time())}.rmdoc")
                ...
                if '[f]' in line and not any(ext in line for ext in ['.pdf', '.epub']):
                    ...
                    if success:
                        ...
                        for root, dirs, files in os.walk("."):
                            for file in files:
                                if file.endswith(".rmdoc") and file != os.path.basename(notebook_path):
                                    logger.info(f"Found downloaded notebook: {os.path.join(root, file)}")
                                    example_path = os.path.join(root, file)
                                    
                                    # Copy to debug dir and analyze
                                    shutil.copy2(example_path, os.path.join(debug_dir, "example_notebook.rmdoc"))
                                    
                                    # Analyze notebook
                                    try:
                                        with zipfile.ZipFile(example_path, 'r') as zip_ref:
                                            files = zip_ref.namelist()
                                            logger.info(f"Example notebook contains: {files}")
                                            ...
                                            if metadata_files:
                                                with zip_ref.open(metadata_files[0]) as f:
                                                    ...
                                                    logger.info(f"Example metadata: {json.dumps(metadata, indent=2)}")
                                                    
                                                    # Update our metadata to match format
                                                    with open(metadata_path, 'w') as f2:
                                                        # Copy format but update values
                                                        for key in metadata:
                                                            if key not in ["visibleName", "lastModified", "lastOpened"]:
                                                                content_data[key] = metadata[key]
                                                        ...
                                                        logger.info(f"Updated metadata to match example format")
                                        ...
                                        logger.error(f"Error analyzing example notebook: {e}")
                    ...
                    for file in files:
                        if file.endswith('.metadata'):
                            ...
                            with open(content_debug_path, 'r') as f:
                                content = json.load(f)
                                logger.info(f"Content file content: {json.dumps(content, indent=2)}")
                except Exception as e:
...
if __name__ == "__main__":
    if create_and_upload_minimal_notebook():
        print(f"Successfully created and uploaded minimal test notebook '{NOTEBOOK_NAME}'")
    else:
