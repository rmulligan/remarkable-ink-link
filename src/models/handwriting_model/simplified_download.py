...
def main():
    ...
    output_dir = os.path.join(home_dir, "dev/remarkable-ink-link/handwriting_model/downloads")
    ...
    try:
        ...
        clean_name = notebook_parts[1].strip() if len(notebook_parts) > 1 else notebook_name
        ...
        download_cmd = [
            ...
            "rmapi", "get", clean_name, "/output/notebook.zip"
        ]
        
        download_result = subprocess.run(download_cmd, capture_output=True, text=True, check=False)
        ...
        if os.path.exists(output_path):
            ...
            extract_cmd = ["unzip", "-o", output_path, "-d", os.path.join(output_dir, "extracted")]
            extract_result = subprocess.run(extract_cmd, capture_output=True, text=True, check=False)
            
            if extract_result.returncode != 0:
                logger.error(f"Failed to extract notebook: {extract_result.stderr}")
                return
            
            logger.info(f"Extracted notebook to {os.path.join(output_dir, 'extracted')}")
        else:
