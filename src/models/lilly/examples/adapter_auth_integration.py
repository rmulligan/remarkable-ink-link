...
def create_authenticated_adapters():
    ...
    if SERVICE_PROTON_CALENDAR in authenticated_services:
        ...
        if credentials:
            logger.info(f"Creating Proton Calendar adapter for {credentials['username']}")
    ...
    if SERVICE_GOOGLE_DRIVE in authenticated_services:
        ...
        if credentials:
            ...
            adapters['google_drive'] = GoogleDriveAdapter(
...
def main():
    ...
    if 'proton_mail' in adapters:
        ...
        try:
            ...
            print(f"Inbox has {inbox.total_messages} messages, {inbox.unread_messages} unread")
