...
def get_unused_imports():
    ...
    result = subprocess.run(['poetry', 'run', 'pyflakes', 'src/'], 
                           capture_output=True, text=True, cwd='/home/ryan/dev/remarkable-ink-link')
...
def fix_import(filename, line_num, import_name):
    ...
    if line.strip() == f"import {import_name}":
        ...
        if new_line.strip() in ['', 'from', 'import'] or re.match(r'^\s*from\s+\S+\s+import\s*$', new_line):
            lines.pop(line_index)
        else:
    ...
    for filename, imports in files_to_fix.items():
        ...
        for imp in imports:
            if fix_import(filename, imp['line_num'], imp['import_name']):
                ...
                print(f"  Failed to fix: {imp['import_name']} on line {imp['line_num']}")
